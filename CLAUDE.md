...
- Ведущий по умолчанию «0 токенов»: готовый банк шуток в `packages/shared/src/jokes.ts` (уровни off/normal/savage). Режим `ai` — опционально живые реплики через LLM (`server/game/src/services/aiHost.ts`, DeepSeek, ключ только в env сервера). Нет ключа / ошибка / таймаут → молча фолбэк на банк, игра не ломается.
...
- `DEEPSEEK_API_KEY` (режим ИИ-ведущего) — там же: Render → Environment, локально в `server/game/.env`. Необязателен: без него ведущий работает на банке шуток. Модель можно переопределить через `DEEPSEEK_MODEL` (по умолчанию `deepseek-v4-flash`).
